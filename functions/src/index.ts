...
export const sendBulkEmail = onCall(
  ...
  async (request) => {
    ...
    const {subject, body, recipientUids} = request.data as {
      subject: string;
      body: string;
      recipientUids: string[] | "all";
    };
    ...
    try {
      ...
      let recipients: {email: string; fullName: string; uid: string}[] = [];
      ...
      const uniqueEmails = new Map<string, {email: string; fullName: string}>();
      recipients.forEach((r) => {
        uniqueEmails.set(r.email.toLowerCase(), {email: r.email, fullName: r.fullName});
      });
      recipients = Array.from(uniqueEmails.values()).map((r) => ({...r, uid: ""}));
