/**
 * Data sync logic - Fetches from Firestore and caches to IndexedDB.
 */
...
async function syncEventConfig(): Promise<void> {
  try {
    // Admin saves to eventConfig/pricing with nights object
    const docRef = doc(db, 'eventConfig', 'pricing');
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      const data = docSnap.data();
      // Store the nights object directly
...
async function syncUserSelections(userId: string): Promise<void> {
  try {
    // Selections are stored in users/{userId}.accommodationSelections
...
async function syncUserProfile(userId: string): Promise<void> {
  try {
    // Find user's registration by querying registrations where uid matches
    const snapshot = await getDocs(collection(db, 'registrations'));
